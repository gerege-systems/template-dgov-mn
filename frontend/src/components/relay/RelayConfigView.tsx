...
import { Loader2, Trash2, Plus, Building2, Route as RouteIcon } from 'lucide-react';
...
export default function RelayConfigView() {
  ...
  const [pf, setPf] = useState({ code: '', name: '', supervisor_contact: '' });
  ...
  const createPlatform = async () => {
    ...
    setPf({ code: '', name: '', supervisor_contact: '' });
      ...
      <section className="card">
        ...
        <div>
          {(platforms.data ?? []).map((p) => (
            <div className="defrow" key={p.id}>
              <span className="defrow__label">{p.name} <span className="muted mono" style={{ fontSize: 12 }}>({p.code})</span></span>
